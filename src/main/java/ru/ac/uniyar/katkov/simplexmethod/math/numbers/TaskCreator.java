...
public class TaskCreator {
    ...
    private static <T extends Num<T>> void fillABMLimits(Arithmetic<T> ametic,T[][]limits, T[][] ABMLimits, int rows, int cols){
        for(int i=0;i<rows;++i){
            if (cols >= 0) System.arraycopy(limits[i], 0, ABMLimits[i], 0, cols);
        }
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < rows; ++j) {
                if (i == j) {
                    ABMLimits[i][j + cols] = ametic.parse("1");
                } else {
                    ABMLimits[i][j + cols] = ametic.zero();
                }
            }
        }
    }
    ...
    public static Task<Doubl> createDoubleABMTaskFromGrid(GridPane gridPane, int rows, int cols) {
        ...
        m.prepareToABM();
        fillABMLimits(ametic,limits, ABMLimits,rows,cols);
        fillABMTaskFunc(ametic,func,rows,cols);

        return new Task<>(func, new Matrix<>(ABMLimits, m.getExtension()));
    }
